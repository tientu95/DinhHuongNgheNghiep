    ...
    <?php
        ...
        ;?>
            <?php 
                ...
                $id_chuyen_nganh = $_GET["id"];

                $sql = "
                          SELECT * 
                          FROM tbl_chuyen_nganh
                          WHERE id_chuyen_nganh = '".$id_chuyen_nganh."'
                ";

                // 3. Thực thi câu lệnh lấy dữ liệu mong muốn
                $chuyen_nganh = mysqli_query($ket_noi, $sql);

                // 4. Hiển thị ra dữ liệu mà các bạn vừa lấy
                $row = mysqli_fetch_array($chuyen_nganh);
            ;?>
                        ...
                        <div class="row justify-content-center">
                            <div class="col-lg-7">
                                ...
                                <div style="background-image: repeating-radial-gradient(0,#179EA1, #1a2936); color: #fff;" class="card-header">
                                    ...
                                    <div class="card-body">
                                        <form method="POST" action="chuyen_nganh_sua_thuc_hien.php" enctype="multipart/form-data">
                                            <div class="form-floating mb-3">
                                                <input class="form-control" id="inputEmail" type="text" placeholder="Tên chuyên ngành" name="txtTen_chuyen_nganh" value="<?php echo $row["ten_chuyen_nganh"];?>" />
                                                <label for="inputEmail">Tên ngành</label>
                                            </div>
                                            <div class="form-floating mb-3">
                                                <select name="txtIDnganh" id="">
                                            <?php 
                                                        // 1. Load file cấu hình để kết nối đến máy chủ CSDL, CSDL
                                                        include("../config.php");
                                                        ...
                                                        while ($row_thi_truong = mysqli_fetch_array($thi_truong)) {
                                                            $i++;
                                                    ;?>
                                                                <option name="txtTTid" value="<?php echo $row_thi_truong["id_nganh_dt"] ;?>">
                                                                    <?php echo $row_thi_truong["ten_nganh_dt"] ;?>
                                                                </option>
                                                                <?php
                                                            }
                                                    ;?>
                                            ...
                                            <div class="form-floating mb-3">
                                                <textarea placeholder="Mô tả" name="txtMoTa" id="txtMoTa"><?php echo $row["mo_ta_cn"];?></textarea>
                                                <label for="inputEmail">Mô tả</label>
                                            </div>
                                            <div class="mt-4 mb-0">
                                                <input type="hidden" name="txtID" value="<?php echo $row["id_chuyen_nganh"];?>">
